    ...
    <div class="bg-green-900 w-200 mx-auto rounded">
            ...
            <form action="{{ route('todo.update', $task->id) }}" method="POST">
                ...
                <div class="bg-amber-900 flex p-2 m-2">
                    ...
                    <input type="text" placeholder="Título" name="title" id="title" value="{{ $task->title }}"class="bg-amber-200 p-2 m-2 flex flex-grow rounded">
                </div>

                <div class="bg-amber-900 min-h-80 p-2 m-2 flex overflow-auto max-h-80">
                    <textarea placeholder="Descrição" name="description" id="description" value="{{ $task->description }}" class="bg-amber-200  p-2 m-2 w-full rounded"></textarea>
                </div>

                <div class="input-container bg-amber-900 p-2 m-2 text-center">
                    <input type="date" name="due_date" id="due_date" value="{{ $task->due_date }}" class="bg-amber-200 m-1 p-2 rounded">
                </div>                                    
